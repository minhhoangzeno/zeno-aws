...
export default function ModalTeam(props: IModalTeam) {
        ...
        if (result.data.leaderId !== null) {
          ...
          AccountAPI.update(result.data.leaderId, {
            ...
            dispatch(
              UpdateTeam({
                ...emptyTeam,
                members: emptyTeam.members?.filter((el) => el.id !== res.data.id),
              })
            );
          });
          ...
          dispatch(
            PutTeam({
              ...result.data,
              members: team?.members ? [...team.members] : [{ ...infoLeader, teamId: result.data.id }],
            })
          );
        } else {
  ...
  return (
    <>
      <Modal
        destroyOnClose={true}
        title={team?.id ? <Typography.Text>Cập nhật Team</Typography.Text> : <Typography.Text>Thêm Team</Typography.Text>}
        style={{ top: 20 }}
        open={modalOpen}
